...
class Board:
    ...
    y_tile = pygame.image.load(os.path.join(IMAGE_DIR, 'ytile.png'))
    dy_tile = pygame.image.load(os.path.join(IMAGE_DIR, 'dytile.png'))
    ...
    def display_board(self, last_move):
        ...
        self.draw_tiles(last_move)

    def draw_tiles(self, last_move):
        ...
        f_rank = 7 - chess.square_rank(last_move.from_square)
        f_file = chess.square_file(last_move.from_square)
        if is_odd(f_rank):
            if is_odd(f_file):
                self.display_surf.blit(Board.y_tile, Board.board_rect[f_rank][f_file])
            else:
                self.display_surf.blit(Board.dy_tile, Board.board_rect[f_rank][f_file])
        elif not is_odd(f_rank):
            if is_odd(f_file):
                self.display_surf.blit(Board.dy_tile, Board.board_rect[f_rank][f_file])
            else:
                self.display_surf.blit(Board.y_tile, Board.board_rect[f_rank][f_file])
        
        t_rank = 7 - chess.square_rank(last_move.to_square)
        t_file = chess.square_file(last_move.to_square)
        if is_odd(t_rank):
            if is_odd(t_file):
                self.display_surf.blit(Board.y_tile, Board.board_rect[t_rank][t_file])
            else:
                self.display_surf.blit(Board.dy_tile, Board.board_rect[t_rank][t_file])
        elif not is_odd(t_rank):
            if is_odd(t_file):
                self.display_surf.blit(Board.dy_tile, Board.board_rect[t_rank][t_file])
            else:
                self.display_surf.blit(Board.y_tile, Board.board_rect[t_rank][t_file])
    ...
    def update_pieces(self, fen, last_move):
